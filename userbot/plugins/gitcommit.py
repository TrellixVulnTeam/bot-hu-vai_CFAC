...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of ULTRONBOT`")
        return
    ULTRONBOT = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await git_commit(downloaded_file_name, ULTRONBOT)


async def git_commit(file_name, ULTRONBOT):
    content_list = []
    ...
    if create_file == True:
        ...
        try:
            ...
            await ULTRONBOT.edit("Cannot Upload Plugin")
    else:
        return await ULTRONBOT.edit("`Committed Suicide`")
