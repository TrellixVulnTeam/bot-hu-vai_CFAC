...
ULTRON_NAME = str(ALIVE_NAME) if ALIVE_NAME else "ULTRON User"
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            try:
                ...
                await edit_or_reply(event, 
                   "✓ **YOUR FILE :-** https://telegra.ph{} \n✓ **Time Taken :-** `{}` secs \n✓ **By :- [{}](tg://user?id={})**".format(
                        media_urls[0], (ms + ms_two), ULTRON_NAME, aura
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            await edit_or_reply(event, 
                  f"✓ **Pasted to** [telegraph]({David99q}) \n✓ **Time Taken :-** `{ms}` secs\n✓** By :- **[{ULTRON_NAME}](tg://user?id={aura})", link_preview=True)
    else:
        await edit_or_reply(event, 
