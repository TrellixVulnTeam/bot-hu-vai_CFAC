...
from ULTRONBOT.utils import admin_cmd, sudo_cmd, progress
from userbot import CMD_HELP
from userbot.cmdhelp import CmdHelp

FF_MPEG_DOWN_LOAD_MEDIA_PATH = "./downloads/ULTRONBOT.media.ffmpeg"
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            ultronevent = await edit_or_reply(event, "`Saving the file...`")
            try:
                c_time = time.time()
                downloaded_file_name = await event.client.download_media(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, ultronevent, c_time, "trying to download")
                    ),
                )
...
async def ff_mpeg_trim_cmd(event):
    ...
    ultronevent = await edit_or_reply(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await edit_delete(
                ultronevent, f"**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, ultronevent, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await edit_delete(ultronevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        ...
        await edit_delete(ultronevent, "RTFM")
        return
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(ultronevent, f"`Completed Process in {ms} seconds`", 3)
