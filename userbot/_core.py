...
ULTRON_logo = "./ULTRONBOT/ULTRONBOT_logo.jpg"
aura = ULTRONBOT.uid
ULTRON = f"[{DEFAULTUSER}](tg://user?id={aura})"

@ULTRONBOT.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@ULTRONBOT.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    omk = f"**⍟ Plugin name ≈** `{input_str}`\n**⍟ Uploaded by ≈** {ULTRON}\n\n⚡ **[Legendary ULTRONBOT](t.me/Its_UltronBot)** ⚡"
...
@ULTRONBOT.on(admin_cmd(pattern="install$", outgoing=True))
@ULTRONBOT.on(sudo_cmd(pattern="install$", allow_sudo=True))
                ...
                if shortname in CMD_LIST:
                    ...
                    for i in CMD_LIST[shortname]:
                        ...
                        if b == 1:
                            ...
                            b = 1
                        await event.edit(a)
                    return await event.edit(f"✅ **Installed module** :- `{shortname}` \n✨ BY :- {ULTRON}\n\n{string}\n\n        ⚡ **[Legendary ULTRONBOT](t.me/Its_UltronBot)** ⚡", link_preview=False)
...
@ULTRONBOT.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@ULTRONBOT.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
...
@ULTRONBOT.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
@ULTRONBOT.on(sudo_cmd(pattern=r"upload (?P<shortname>\w+)$", allow_sudo=True))
...
@ULTRONBOT.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
@ULTRONBOT.on(sudo_cmd(pattern=r"load (?P<shortname>\w+)$", allow_sudo=True))
